...
async fn main() {
    ...
    let proxies_html = generate_proxies_html(&card_images.into_iter().flat_map(|(_, card_images)| card_images).collect(), &args.extra_cards).expect("Could not generate proxies HTML content");
